...
public class SpaceMission {
    ...
    private static Pair calculateAcceleration(final Particle currentParticle,
        ...
        for (Map.Entry<Particle, R> entry : initialStates.entrySet()) {
            ...
            if (p != currentParticle) {
                double deltaR0x = currentParticleR0.getX() - rs.get(R0.ordinal()).getX();
                double deltaR0y = currentParticleR0.getY() - rs.get(R0.ordinal()).getY();
                double distance = pow((deltaR0x), 2) + pow((deltaR0y), 2);
                double ex = deltaR0x / distance;
                double ey = deltaR0y / distance;
                fx += G * currentParticle.getMass() * p.getMass() * ex / distance;
                fy += G * currentParticle.getMass() * p.getMass() * ey / distance;
            }
        }
    ...
    public static Map<Particle, R> predict(Map<Particle, R> currentRs, final double dt) {
        ...
        final Map<Particle, R> newPredictionsMap = new HashMap<>();
        ...
        currentRs.forEach((p, r) -> {
            ...
            newPredictionsMap.put(p, newPredictions);
        });

        return newPredictionsMap;
    }
